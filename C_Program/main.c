...
void deroulement_partie(plateau parking){
    coordonnee vehicule_a_deplacer = {0, 0};
    do {
        vehicule_a_deplacer.ligne = ligne_utilisateur();
        vehicule_a_deplacer.colonne = colonne_utilisateur();
    }while(case_contient_vehicule(parking, vehicule_a_deplacer) );
}
...
void display_one_line_parking(plateau parking, int line){
    char letter = 65 + line;
    for (int i = 0; i < parking.nbLignes; i++) {
        if (i == 0){
            printf("%c [%c]",letter,parking.listeVehicule[line][i].symbole);
        } else{
            printf("[%c]",parking.listeVehicule[line][i].symbole);
        }
    }
    printf("\n");
}

void display_parking(plateau parking){
    display_index_column(parking.nbColonnes);
    for (int i = 0; i < parking.nbLignes; i++) {
        display_one_line_parking(parking, i);
    }
}
